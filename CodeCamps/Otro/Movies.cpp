...
bool Movies::add_movie(std::string name, std::string rating, int watched) {
    bool state {true};
    for (auto i : this->movies) if (i.get_name() == name) state = false;
    if (state == true) {
        Movie pending {name, rating, watched};
        this->movies.push_back(pending);
    return state;
}
...
bool Movies::increment_watched(std::string name) {
   ...
   if (state == true) this->movies[index].set_watched(this->movies[index].get_watched()+1);
   return state;
}
...
void Movies::display() const {
   for (auto i : this->movies) i.display();
}